...
The functions all follow this basic schema whereby all the objects from the previous stage are arguments in the subsequent functions, whether or not they are actually used in the calculations within the function:
...
States are represented as rasters (`SpatRaster`) with any resolution, extent or CRS (or no CRS). We simulate the state of the environment and 'target' separately. The state simulation functions take a raster which is refereed to as the 'background', the resolution, extent and CRS is inherited from this when simulating the state.
